...
library(htmlwidgets)
library(scales)
list.files (path = "C:/Users/Aleks/Desktop/uni/data science/the office")
...
Contiene tutte le linee di dialogo dette nelle 9 stagioni di the Office organizzate per persoanggio ed episodio.
Sono presenti anche i ratings ricevuti su IMDB per i vari episodi.  
![](C:/Users/Aleks/Desktop/uni/data science/the office/images/the_office_inizio.png)
...
The Office segue le vite di ogni giorno dei dipendenti della Dunder Mifflin-Scranton, guidati dal direttore della filiale Michael Scott.  
![](C:/Users/Aleks/Desktop/uni/data science/the office/images/Worlds_best_boss.jpg)
...
e molte altre cose...  
![](C:/Users/Aleks/Desktop/uni/data science/the office/images/the-office-kevin.jpg)
...
![](C:/Users/Aleks/Desktop/uni/data science/the office/images/prison-mike.jpg)  
...
ggplotly(c)
```
Come vediamo __Michael__ e quello piu presente in tutti gli episodi tranne che per le ultime 2 stagioni dove fa solo da comparsa  
...
#filter(character%in% c("Michael","Dwight","Jim","Andy","Angela","Pam"))%>%
  ggplot(aes(interaction(character,sentiment,sep=':'), freq, fill = character)) +
  geom_col(show.legend = FALSE)+
coord_flip()
#fare solo il sentimento con numero piu alto per personaggio
```
...
bigrams_united <- bigrams_filtered %>%
 ...
 dio <-bigrams_united %>% 
  count(bigram, sort = TRUE) 
  # evaluate an R expression in an environment constructed from data
 # with(wordcloud(bigram, n, max.words = 70,random.order = FALSE, colors=brewer.pal(8, "Dark2")))
...
zio<-trigrams_united %>% 
  ...
  # evaluate an R expression in an environment constructed from data
 # with(wordcloud(trigram, n, max.words = 100,random.order = FALSE, colors=brewer.pal(8, "Dark2")))
...
bigram_tf_idf <- bigrams_united %>%
  ...
  filter(character %in% c("Michael","Jim","Dwight","Pam","Andy","Erin"))%>%
...
![](C:/Users/Aleks/Desktop/uni/data science/the office/images/nard-dog.jpg)
...
sum(str_count(Transcripts$text, "Thats what he said"))+
sum(str_count(Transcripts$text, "THATS WHAT HE SAID"))
#sprintf("durante il corso della serie viene detto %i volte", number)

```
Durante il corso dell'intera serie viene detto  __`r number`__ volte  

  ![](C:/Users/Aleks/Desktop/uni/data science/the office/images/she_said.jpg)
...
#word_cors %>%
 # filter(item1 == c("um","yeah","uh","hey","mm"))
```
...
![](C:/Users/Aleks/Desktop/uni/data science/the office/images/Dwight_girl.jpg)
...
p<-ggplot(lol, aes(ep_key, imdb_rating,color=season)) +
  ...
  geom_smooth() +### perche geom smooth non va?
